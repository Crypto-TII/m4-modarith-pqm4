...
#include <fp2.h>
#include "code_MFP248_mont.c"

const digit_t p[NWORDS_FIELD] = { 0xffffffff, 0xffffffff, 0xffffffff, 0xffffffff,
                                  0xffffffff, 0xffffffff, 0xffffffff, 0x4ffffff };

const digit_t ZERO[NWORDS_FIELD] = { 0, 0, 0, 0, 0, 0, 0, 0 };

const digit_t ONE[NWORDS_FIELD] = { 0x00000033, 0x00000000, 0x00000000, 0x00000000,
                                    0x00000000, 0x00000000, 0x00000000, 0x01000000 };

static const digit_t TWO_INV[NWORDS_FIELD] = { 0x00000019, 0x00000000, 0x00000000, 0x00000000,
                                               0x00000000, 0x00000000, 0x00000000, 0x03000000 };

static const digit_t THREE_INV[NWORDS_FIELD] = { 0xaaaaaabb, 0xaaaaaaaa, 0xaaaaaaaa, 0xaaaaaaaa,
                                                 0xaaaaaaaa, 0xaaaaaaaa, 0xaaaaaaaa, 0x03aaaaaa };

// Montgomery representation of 2^512
static const digit_t R2[NWORDS_FIELD] = { 0x33333d70, 0x33333333, 0x33333333, 0x33333333,
                                          0x33333333, 0x33333333, 0x33333333, 0x33333333 };

static const digit_t PM1O3[NWORDS_FIELD] = { 0xaaaaaaaa, 0xaaaaaaaa, 0xaaaaaaaa, 0xaaaaaaaa,
                                             0xaaaaaaaa, 0xaaaaaaaa, 0xaaaaaaaa, 0x01aaaaaa };
                                    
void
fp_set_small(fp_t *x, const digit_t val)
{
    modint_MFP248_ct((int)val, *x);
}

NOINLINE void
fp_mul_small(fp_t *x, const fp_t *a, const uint32_t val)
{
    modmli_MFP248_ct(*a, (int)val, *x);
}
...
{
    return -(uint32_t)modcmp_MFP248_ct(*a, *b);
}

uint32_t
fp_is_zero(const fp_t *a)
{
    return -(uint32_t)modis0_MFP248_ct(*a);
}
...
{
    modcsw_MFP248_ct((int)(ctl & 0x1), *a, *b);
}
...
fp_inv(fp_t *x)
{
    modinv_MFP248_ct(*x, NULL, *x);
}

uint32_t
fp_is_square(const fp_t *a)
{
    return -(uint32_t)modqr_MFP248_ct(NULL, *a);
}
...
fp_half(fp_t *out, const fp_t *a)
{
    ...
    modcpy_MFP248_ct(*a, *out);
    modshr_MFP248_ct(1, *out);
    (*out)[7] += 0x02800000 & ~ctl;
}
#else
void
fp_half(fp_t *out, const fp_t *a)
{
    modmul_MFP248_ct(TWO_INV, *a, *out);
}
#endif
...
{
    modmul_MFP248_ct(THREE_INV, *a, *out);
}
#endif
...
fp_decode(fp_t *d, const void *src)
{
    ...
    for (i = BITS / 8 - 1; i >= 0; i--) {
        modshl_MFP248_ct(8, *d);
        (*d)[0] += (spint)b[i];
    }
    res = (spint)-modfsb_MFP248_ct(*d);
    nres_MFP248_ct(*d, *d);
