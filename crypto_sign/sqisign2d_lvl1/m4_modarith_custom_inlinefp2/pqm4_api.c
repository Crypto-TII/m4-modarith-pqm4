...
#ifdef INSTRUMENT_SQISIGN
#include <hal.h>
#include <stdio.h>

int modadd_count = 0, modsub_count = 0, modmul_count = 0, modsqr_count = 0;
...
#ifdef INSTRUMENT_SQISIGN
  PRINT_VARIABLE(modadd_count);
  PRINT_VARIABLE(modsub_count);
  PRINT_VARIABLE(modmul_count);
  PRINT_VARIABLE(modsqr_count);
#endif
  if (mlen) {
    *mlen = mlen_ull;
  }
